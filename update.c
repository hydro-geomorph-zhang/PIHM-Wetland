...
update(double t, void *DS)
{
	...
	if (MD->Tide_mode == 1)
	{
		for (k = 0; k < MD->NumTide; k++) {
			while (MD->TSD_Tide[k].iCounter < MD->TSD_Tide[k].length && t / (UNIT_C) > MD->TSD_Tide[k].TS[MD->TSD_Tide[k].iCounter + 1][0]) {
				MD->TSD_Tide[k].iCounter++;
			}
		}
	}
